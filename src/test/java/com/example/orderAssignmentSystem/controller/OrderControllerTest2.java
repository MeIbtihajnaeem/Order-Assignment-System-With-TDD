...
public class OrderControllerTest2 {
	...
	@Test
	public void testModifyOrderMethodWhenOrderNotChanged() {
		final String orderDescription = "abcdefghijklmnopqrstuvwxyzasdf123";
		...
		Worker worker = new Worker();
		worker.setWorkerId(workerId);
		CustomerOrder oldOrder = new CustomerOrder(orderId, CategoryEnum.PLUMBER, orderDescription,
				OrderStatusEnum.PENDING, worker);
		CustomerOrder updatedOrder = new CustomerOrder(orderId, CategoryEnum.PLUMBER, orderDescription,
				OrderStatusEnum.PENDING, worker);
		...
		InOrder inOrder = inOrder(orderRepository, orderView, workerRepository);
		inOrder.verify(orderView).showError("Cannot modify order because the order remains the same", updatedOrder);
		verifyNoMoreInteractions(ignoreStubs(orderRepository));
	}
